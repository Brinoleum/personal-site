...
Large language models such as BERT and the GPT family are commonly used for natural language processing tasks, where the input is a sequence of word or subword tokens and the model is trained to predict missing or following tokens given some context. The underlying architectures, however, are not specific to English text: any sequential data with a sufficiently large training corpus can in principle be modeled in the same way. Genomic sequence data is one such input, and there are now several families of genomic language models — DNA-BERT, DNA-GPT, HyenaDNA, and Caduceus among others — that adapt these architectures to operate on nucleotide sequences. Benchmarks on these models show that they perform well on tasks such as species classification, distinguishing between human and worm or between primates from sequence alone.
